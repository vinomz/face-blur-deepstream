...
static void
addFaceProposal(const float* output, size_t channelsSize, uint netW, uint netH, size_t n, NvDsInferInstanceMaskInfo& b)
{
  size_t landmarksSize = 15;
  b.mask = new float[landmarksSize];
  for (size_t p = 0; p < landmarksSize / 3; ++p) {
    if (channelsSize - 5 == 10) {
      b.mask[p * 3 + 0] = clamp(output[n * channelsSize + p * 2 + 5], 0, netW);
      b.mask[p * 3 + 1] = clamp(output[n * channelsSize + p * 2 + 6], 0, netH);
      b.mask[p * 3 + 2] = 1.0;
    }
    else {
...
static std::vector<NvDsInferInstanceMaskInfo>
decodeTensorYoloFace(const float* output, size_t outputSize, size_t channelsSize, uint netW, uint netH,
    const std::vector<float>& preclusterThreshold)
{
  ...
  for (size_t n = 0; n < outputSize; ++n) {
    ...
    addFaceProposal(output, channelsSize, netW, netH, n, b);
    ...
    std::vector<NvDsInferInstanceMaskInfo>& objectList)
{
  ...
  std::vector<NvDsInferInstanceMaskInfo> objects = decodeTensorYoloFace((const float*) (output.buffer), outputSize,
